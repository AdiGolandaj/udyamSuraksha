...
import { PrismaClient, type SensitivityType } from "@prisma/client";
...
async function main() {
  ...
  const kiranaItems: Array<{ name: string; category: string; qty: number; unit: string; val: number; sens: SensitivityType[] }> = [
  ...
  const pharmacyItems: Array<{ name: string; category: string; qty: number; unit: string; val: number; sens: SensitivityType[] }> = [
  ...
  const electronicsItems: Array<{ name: string; category: string; qty: number; unit: string; val: number; sens: SensitivityType[] }> = [
  ...
  const rainfallByMonth: Record<number, number> = {
  ...
  const floodIncidentsByMonth: Record<number, number> = {
    0: 0, 1: 0, 2: 0, 3: 0, 4: 1, 5: 2, 6: 3, 7: 4, 8: 3, 9: 2, 10: 1, 11: 0
  };
  const powerOutagesByMonth: Record<number, number> = {
    0: 1, 1: 0, 2: 0, 3: 1, 4: 2, 5: 3, 6: 4, 7: 5, 8: 4, 9: 3, 10: 2, 11: 1
  };
  const transportDisruptByMonth: Record<number, number> = {
    0: 0, 1: 0, 2: 0, 3: 0, 4: 1, 5: 2, 6: 2, 7: 3, 8: 3, 9: 2, 10: 1, 11: 0
  };

  const trendRows: Array<{
    regionCode: string; trendType: string; value: number; unit: string; recordedAt: Date; source?: string
  }> = [];
  ...
  for (const region of trendRegions) {
    for (let m = 11; m >= 0; m--) {
      ...
      trendRows.push(
        { regionCode: region, trendType: "rainfall",              value: Math.round(rainfallByMonth[11 - m] * scaleFactor), unit: "mm",       recordedAt: date, source: "IMD" },
        { regionCode: region, trendType: "flood_incident",        value: floodIncidentsByMonth[11 - m],                      unit: "incidents", recordedAt: date, source: "LRDB Report" },
        { regionCode: region, trendType: "power_outage",          value: powerOutagesByMonth[11 - m] * (region === REGION_BHOR ? 1.5 : 1),    unit: "incidents", recordedAt: date, source: "MSEDCL" },
        { regionCode: region, trendType: "transport_disruption",  value: transportDisruptByMonth[11 - m],                    unit: "incidents", recordedAt: date, source: "MSRTC" },
        { regionCode: region, trendType: "customer_activity",     value: Math.max(20, 100 - floodIncidentsByMonth[11 - m] * 15), unit: "index", recordedAt: date, source: "Manual" },
      );
    }
  }
